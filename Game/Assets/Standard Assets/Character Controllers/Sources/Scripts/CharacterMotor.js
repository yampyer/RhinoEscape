...
private function UpdateFunction () {
	...
	if (grounded && !IsGroundedTest() && !dead) {
		grounded = false;
	...
	else if (!grounded && IsGroundedTest() && !dead) {
		grounded = true;
		//animator.SetBool("grounded", grounded);
...
function OnControllerColliderHit (hit : ControllerColliderHit) {
	if (hit.gameObject.CompareTag("Enemies")){
	dead = true;
	...
	if (hit.normal.y > 0 && hit.normal.y > groundNormal.y && hit.moveDirection.y < 0) {
		if ((hit.point - movement.lastHitPoint).sqrMagnitude > 0.001 || lastGroundNormal == Vector3.zero) {
			...
			//if (hit.gameObject.CompareTag("Enemies")) Destroy(hit.gameObject);
		} else {
...
function CalculateJumpVerticalSpeed (targetJumpHeight : float) {
	...
	return Mathf.Sqrt (0.5 * targetJumpHeight * movement.gravity);
}
...
function DisplayRestartButton()
{
    if (dead)
    {
        var buttonRect : Rect = Rect(Screen.width * 0.35f, Screen.height * 0.45f, Screen.width * 0.30f, Screen.height * 0.1f);
